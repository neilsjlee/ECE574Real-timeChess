            ...
            if event.type == self.pg.MOUSEBUTTONDOWN:
                if self.m.playing and BOARD_MARGIN_X + SQUARE_WIDTH * 8 >= event.pos[0] >= BOARD_MARGIN_X and BOARD_MARGIN_Y + SQUARE_WIDTH * 8 >= event.pos[1] >= BOARD_MARGIN_Y:
                    if event.button != 3:
                        ...
                        target = self.m.board[self.m.target_square[1]][self.m.target_square[0]]
                        # if target and self.m.player_color_is_black == target.colour:
                        if target:
                            # Modified above 1 line(s)
                            self.m.legal_moves = target.find_moves(self.m.board, self.m.target_square, kings, check)
                    elif target_square and target:
                        true_target, destination = self.find_square(event.pos[0], event.pos[1])
